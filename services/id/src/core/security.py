...
def require_internal_signature(request) -> None:
    ...
    if not request_id:
        raise HttpError(
            ...
            cast(
                Any,
                {"code": "MISSING_REQUEST_ID", "message": "X-Request-Id is required"},
            ),
        )
    ...
    if not ts or not sig:
        raise HttpError(
            401,
            cast(
                Any, {"code": "UNAUTHORIZED", "message": "Missing internal signature"}
            ),
        )
    ...
    try:
        ...
        raise HttpError(
            401,
            cast(
                Any, {"code": "UNAUTHORIZED", "message": "Invalid internal timestamp"}
            ),
        ) from exc
    ...
    if abs(now - ts_int) > 300:
        raise HttpError(
            401,
            cast(
                Any, {"code": "UNAUTHORIZED", "message": "Internal signature expired"}
            ),
        )
    ...
    if not hmac.compare_digest(str(sig), expected):
        raise HttpError(
            401,
            cast(
                Any, {"code": "UNAUTHORIZED", "message": "Invalid internal signature"}
            ),
        )
